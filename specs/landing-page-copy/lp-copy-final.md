# Climate case lab - full copy to the LP

## CLIMATE LITIGATION ACROSS BORDERS

<!-- ── SLOT: Landing uppertab › #about ──────────────────────────────
  This H2 doubles as the site HERO TITLE (render at --ts-display) and heads the About subtab.
  Body blocks below, in order:
    ¶1  lede — what + why
    ¶2  why we share / collaboration ethos
    ¶3  closing invitation
    authorship footer (Lucas, Gustavo)
  GAPS vs spec §2.3: no copy yet for the "What you'll find here" 3 tiles, nor the optional pull-quote.
-->
...
Gustavo Rodrigues, L.B. (USP), independent researcher in computer sciences.

## METHODOLOGY

<!-- ── SLOT: Landing uppertab › #methodology ────────────────────────
  This H2 → the Methodology section heading (spec §2.3 names it "Research Methodology").
  HEADER HIERARCHY — the page MUST render the markdown nesting (Gustavo, 2026-06-16):
     H2 METHODOLOGY .......... section title      (one level)
       └ ### subsections ..... nested subsection headings (one level down)
  Subsections, in order:
     ### Definitions and classification   (includes the six-category list)
     ### The processing pipeline          (anchor for the simplified 4-box pipeline SVG, spec §2.3)
     ### Refinement of the methodology
  "Full methodology paper: forthcoming." stands in for the dropped /methodology.html deep-link (spec §2.3, l.99).
-->
...
<!-- ── SLOT: Landing — Acknowledgements ──────────────────────────────
  Own section, placed AFTER Methodology and BEFORE Collaboration (Gustavo, 2026-06-16).
  NOT in spec §2.2's three-subtab list → the IA needs an amendment: either a 4th subtab
  chip ("Acknowledgements") or an untabbed credits block sitting just above Collaboration.
-->

ethe Sabin Center for Climate Change Law at Columbia Law School in building and maintaining the Climate Case Chart. Every piece of information used here, and every link to case law, traces back to that database. 
...
<!-- ── SLOT: Landing uppertab › #collaboration ───────────────────────
  "Spotted an error? Got an improvement?" → section H1 (spec §2.3).
  Contact is mailto only — no submission form (spec §12.2).
-->
